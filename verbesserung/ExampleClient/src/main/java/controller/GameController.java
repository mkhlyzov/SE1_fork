...
public class GameController {
    ...
    public void startNewGame() {
        ...
        model.setGameHelper(
                new GameHelper(network.getPlayerId()));
    ...
    private void waitPermissionForSendingHalfmap() {
        ...
        while (!canSendMap
            ...
            if (state == null) {
                LOGGER.warning(
                        "GameState ist null. "
                                + "Warte weiter auf Erlaubnis zum Senden der HalfMap.");
                continue;
            }
            ...
            if (myPlayer == null) {
                LOGGER.warning(
                        "Eigener Spieler wurde im GameState "
    ...
    private boolean canSendHalfMap(
            PlayerState player) {
        ...
        switch (player.getState()) {
            ...
            case MustAct -> {
                LOGGER.info(
                        "Erlaubnis zum Senden "
                                + "der HalfMap erhalten.");
                return true;
            }
