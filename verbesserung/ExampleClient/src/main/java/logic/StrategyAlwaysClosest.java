...
public class StrategyAlwaysClosest implements IStrategy {
  ...
  public PlayerMove calculateNextMove(GameHelper gameHelper) {
    ...
    if (playerHasTreasure) {
      ...
      if (goal == null) {
        ...
        goal = findClosestUndiscoveredNode(gameHelper, enemyXmin, enemyYmin, enemyXmax, enemyYmax);
      } else {
      ...
      if (goal == null) {
        ...
        goal = findClosestUndiscoveredNode(gameHelper, myXmin, myYmin, myXmax, myYmax);
      } else {
  ...
  private boolean isSafeMove(FullMap map, int x, int y) {
    return map.getMapNodes().stream()
        .anyMatch(n -> n.getX() == x && n.getY() == y && n.getTerrain() != ETerrain.Water);
  }
  ...
  private FullMapNode findClosestUndiscoveredNode(
      GameHelper gameHelper, int x1, int y1, int x2, int y2) {
    ...
    while (!queue.isEmpty()) {
      ...
      for (int[] d : dirs) {
        ...
        if (!gameHelper.isObserved(neighbor)) {
          if (nx >= x1 && nx < x2 && ny >= y1 && ny < y2) return neighbor;
        }
      }
    }
