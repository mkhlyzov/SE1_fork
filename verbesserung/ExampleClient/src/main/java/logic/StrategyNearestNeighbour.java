...
public class StrategyNearestNeighbour implements IStrategy {
  ...
  public PlayerMove calculateNextMove(GameHelper gameHelper) {
    ...
    if (hasTreasure && peek != null && insideMine(peek)
        || (enemyFortVisible
            && (peek == null || peek.getFortState() != EFortState.EnemyFortPresent))) {
      plannedTour.clear();
    }
      ...
      if (goals.isEmpty()) {
        for (FullMapNode n : map.getMapNodes()) {
          if (!helper.isObserved(n)
              ...
              && n.getTerrain() != ETerrain.Mountain
              && insideEnemy(n)) {
            goals.add(n);
          }
        }
  ...
  List<FullMapNode> bestNearestNeighbourTour(
  ...
  private List<FullMapNode> nearestNeighbourTour(
      FullMap map, FullMapNode start, Set<FullMapNode> unvisited) {
